...
export async function getDashboardSummary({ sheetTitle = 'Pagina1', role = 'admin' } = {}) {
  const now = new Date()
  ...
  const normalizedRole = typeof role === 'string' && role ? role : 'admin'
  const isAdmin = normalizedRole === 'admin'
  const fallbackView = roleViews.find(view => view.id === normalizedRole)
  const effectiveRoleViews = isAdmin
    ? roleViews
    : fallbackView
      ? [fallbackView]
      : [
          {
            ...
            summary: 'Nenhuma visão configurada para este perfil.',
            highlights: [],
            metrics: [],
            focus: [],
            actions: [],
  ...
  const allowedActionIds = isAdmin
    ? new Set(actions.map(action => action.id))
    : new Set(effectiveRoleViews.flatMap(view => view.actions ?? []))

  const filteredActions = actions.filter(action => allowedActionIds.has(action.id))
